...
// Prepare and execute the SQL query to retrieve the statuses and equipment details
...
// Fetch the statuses and equipment details of the request equipment
...
if (array_diff($statuses, [1, 6]) === []) {
    ...
    if ($stmtDelete->affected_rows > 0) {
        ...
        $connection->rollback();
        echo json_encode(['success' => false, 'error' => 'Failed to delete the request.']);
    }
} else {
    // Return a response indicating that the deletion cannot proceed due to conflicting statuses
    echo json_encode(['success' => false, 'error' => 'Cannot delete the request. Conflicting statuses exist.']);
}
