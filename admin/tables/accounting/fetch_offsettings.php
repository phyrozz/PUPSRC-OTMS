...
$offsettingsQuery = "SELECT offsetting_id, CONCAT(DATE_FORMAT(FROM_UNIXTIME(SUBSTRING(offsetting_id, 4)), '%M %e, %Y %l:%i:%s %p')) AS formatted_offsetting_id, DATE_FORMAT(timestamp, '%M %e, %Y') as formatted_timestamp, users.first_name, users.last_name, users.middle_name, users.extension_name, 
...
if (!empty($searchTerm)) {
    $offsettingsQuery .= " AND (offsetting_id LIKE '%$searchTerm%'
                        OR DATE_FORMAT(FROM_UNIXTIME(SUBSTRING(offsetting_id, 4)), '%M %e, %Y %l:%i:%s %p') LIKE '%$searchTerm%'
                        OR DATE_FORMAT(timestamp, '%M %e, %Y') LIKE '%$searchTerm%'
